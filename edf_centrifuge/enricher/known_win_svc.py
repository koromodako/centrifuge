"""Known windows service enricher"""

from json import loads

from ..atom import Other
from ..helper.asyncpg import (
    asyncpg_cleanup_ctx_impl,
    asyncpg_ctx_connection,
    asyncpg_fetch,
)
from ..helper.logging import get_logger
from ..record import Record
from . import (
    Enricher,
    EnricherConfig,
    EnricherContext,
    Feedback,
    register_enricher,
)

GUID = 'known_win_svc'
_LOGGER = get_logger('enricher.known_win_svc')
_QUERY = '''
SELECT tags, tool FROM centrifuge.known_win_svc
WHERE (pattern = false AND service = $1) OR (pattern = true AND $1 LIKE service)
'''


async def _enrich_other_impl(
    ctx: EnricherContext, other: Other, _feedback: Feedback
) -> Record:
    tags = set()
    tools = set()
    connection = asyncpg_ctx_connection(ctx)
    async for e_record in asyncpg_fetch(connection, _QUERY, other.value):
        tags.update(loads(e_record['tags']))
        tools.add(e_record['tool'])
    return {'tags': list(sorted(tags)), 'tools': list(tools)}


_FIELDS = ('tags', 'tools')
_ENRICHER = Enricher(
    guid=GUID,
    fields=_FIELDS,
    enrich_impl_map={
        Other: _enrich_other_impl,
    },
    cleanup_ctx_impl=asyncpg_cleanup_ctx_impl,
)
register_enricher(_ENRICHER, EnricherConfig)
